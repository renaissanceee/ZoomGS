...
def main(input_dir, index):
    ...
    save_file = os.path.join(input_dir, f'real_metrics_{index}.txt')
    print("see metrics in: ", save_file)

    input_dir = os.path.join(input_dir, index, '*.png')
    input_paths = sorted(glob.glob(input_dir, recursive=True))

    sf = open(save_file, 'a')
    sf.write(f'input address:\t{input_dir}\n')
    p = sf.tell()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default="ckpt/zoomgs/01/train/zoom_sequences", help="sequence")

    args = parser.parse_args()
    for view_index in os.listdir(args.model):
        if os.path.isdir(os.path.join(args.model,view_index)):# only folder
            main(args.model, view_index)
