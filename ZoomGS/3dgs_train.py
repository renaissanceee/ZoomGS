...
def training(dataset, opt, pipe, args):
    ...
    viewpoint_stack_uw = viewpoint_stack[:10]
    viewpoint_stack_wide = viewpoint_stack[10:]
    print("num of far:near = 10:", len(viewpoint_stack_wide))
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        viewpoint_cam_uw = viewpoint_stack_uw[(randint(0, len(viewpoint_stack_uw)-1))]
        ...
        # uw
        render_pkg = render(viewpoint_cam_uw, gaussians, pipe, background, info=None)
        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        gt_image = viewpoint_cam_uw.original_image.cuda()
        Ll1 =  l1_loss(image, gt_image)
        loss1 = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)))
        # wide
        render_pkg = render(viewpoint_cam_wide, gaussians, pipe, background, info=None)
        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        gt_image = viewpoint_cam_wide.original_image.cuda()
        Ll1 =  l1_loss(image, gt_image)
        loss2 = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)))
